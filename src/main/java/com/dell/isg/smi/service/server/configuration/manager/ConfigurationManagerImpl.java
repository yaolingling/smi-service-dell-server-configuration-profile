...
public class ConfigurationManagerImpl implements IConfigurationManager {
    ...
    public XmlConfig importConfiguration(ServerAndNetworkShareRequest request) throws Exception {
        ...
        ApplyXmlConfigCmd xmlConfig = new ApplyXmlConfigCmd(request.getServerIP(), request.getServerUsername(), request.getServerPassword(), Integer.toString(request.getShareType()), request.getShareName(), request.getShareAddress(), request.getFileName(), request.getShareUsername(), request.getSharePassword(), request.getShutdownType());
    ...
    @Override
    public XmlConfig exportConfiguration(ServerAndNetworkShareRequest request) throws Exception {

        ExportXmlConfigCmd xmlConfig = new ExportXmlConfigCmd(request.getServerIP(), request.getServerUsername(), request.getServerPassword(), Integer.toString(request.getShareType()), request.getShareName(), request.getShareAddress(), request.getFileName(), request.getShareUsername(), request.getSharePassword(), StringUtils.join(request.getComponentNames(), ","));
        XmlConfig config = xmlConfig.execute();
    ...
    /**
     * This Method extracts the List of ServerComponents for a given Server and Network share request
     * 
     * @param request
    ...
    /**
     * This method gives the list of ServerComponents for a given SystemConfiguration and ServerAndNetworkSharerequest
     * 
     * @param systemConfig- The SystemConfiguration object of XML Configuration file
    ...
    /**
     * This Method updates the XML Configuration File for a given Component Lists to be updated
     * 
     * @param request- The ComponentList request
    ...
    private List<ServerComponent> updateConfigurationFile(ComponentList request) throws Exception {
        ...
        try {
            if (null != request) {
                ...
                JAXBContext jaxbContext = JAXBContextFactory.createContext(new Class[] { SystemConfiguration.class }, null);
    ...
    /**
     * This Method updates the XML SystemConfiguration for a given SystemConfiguration object and ComponentList request
     * 
     * @param systemConfig- The SystemConfiguration object
     * @param request- The ComponentList
     * @return the updated list of ServerComponents
     * @throws Exception
     */
    private List<ServerComponent> updateXMLSystemConfiguration(SystemConfiguration systemConfig, ComponentList request) throws Exception {
    ...
    public Boolean initializeSCPService(ServerAndNetworkShareRequest request) throws Exception {
        ...
        if (mounted && !ConfigurationUtils.fileExist(request.getFilePathName())) {
        	...
        	logger.info("initializeSCPService: File already exist. Skipping export of file to "+ request.getFilePathName());
        }
        return mounted;
    }
    ...
    /**
     * This Method updates the network share filepathName, In case of blank ShareAddress file name will be unique otherwise uses the requested filePathName
     * 
